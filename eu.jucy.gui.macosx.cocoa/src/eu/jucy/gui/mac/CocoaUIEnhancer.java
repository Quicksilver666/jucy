...
public class CocoaUIEnhancer implements IStartup {
    ...
    public void earlyStartup() {
        ...
        display.syncExec(new Runnable() {
            public void run() {
         //   	DCClient.logger.info("in startup");
            	try {
			...
			//	DCClient.logger.info("End register");
			}
	...
	protected void modifyWindowShell(IWorkbenchWindow window) {
		...
		if (coolBarInitiallyVsible || perspectiveBarInitiallyVsible) {
			...
			NSView view = null;
			try {
				view = (NSView) Class.forName("org.eclipse.swt.widgets.Shell").getField("view").get(shell);
			} catch (Exception e1) {
				log(e1);
			} 
	...
	void preferencesMenuItemSelected() {
		runCommand("eu.jucy.gui.preferences");//$NON-NLS-1$
		
	}

	void aboutMenuItemSelected() {
		runCommand("org.eclipse.ui.help.aboutAction");//$NON-NLS-1$
	}
