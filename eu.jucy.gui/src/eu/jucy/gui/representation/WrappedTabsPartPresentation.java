...
//	public static final String MENU_ID = "eu.jucy.presentation";
	...
	private ISystemMenu systemMenuManager = new UCEditorSystemMenu();
...
//		//systemMenuManager.add(new Separator());
//		//HubContributionItem hci = new HubContributionItem();
//	//	systemMenuManager.add(new UCEditorContributionItem());
//		
//		//UCEditor.createContextPopups(getSite(), MENUID, null, null);
//		//hci.initialize(PlatformUI.getWorkbench().getActiveWorkbenchWindow());
//		//systemMenuManager.add(hci);
//	
//
...
//	}
	...
	protected void initTab(ToolItem item, IPresentablePart part) {
		...
		if (showIconOnTabs && getMatchingImage(part.getTitleImage(),item) != item.getImage()) {
			item.setImage(getMatchingImage(part.getTitleImage(),item));
		}
		
	}
	
	private Image getMatchingImage(Image originalImage,ToolItem item) {
		Image replacement = correctlySizedImage.get(originalImage);
		if (replacement == null) {
			Rectangle rect = originalImage.getBounds();
			if (rect.width == CorrectSize && rect.height == CorrectSize) {
				replacement = originalImage;
			} else {
				Image newImage = new Image(originalImage.getDevice(),CorrectSize,CorrectSize);
				...
				gc.drawImage(originalImage, 0, 0, rect.width,rect.height,0, 0, CorrectSize, CorrectSize);
				gc.dispose();
				
				ImageData id = newImage.getImageData();
				id.transparentPixel = id.getPixel(0, 0);
				Image transp = new Image(originalImage.getDevice(), id);
				newImage.dispose();
				
				replacement = transp;
			}
			correctlySizedImage.put(originalImage, replacement);
		}
		return replacement;
	}
	...
	public void showSystemMenu(Point displayPos) {
		systemMenuManager.show(getControl(), displayPos, getCurrent());
