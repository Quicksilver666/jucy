...
public abstract class AbstractNotificationPopup extends Window {
	...
	protected static final long DEFAULT_DELAY_CLOSE = 6 * 1000;
	...
	public AbstractNotificationPopup(Display display,long delayClose) {
		this(display, SWT.NO_TRIM | SWT.ON_TOP | SWT.NO_FOCUS | SWT.TOOL,1.0f,delayClose);
	}

	public AbstractNotificationPopup(Display display, int style,float percentTime,long delayClose) {
	...
	protected void scheduleAutoClose() {
		if (delayClose > 0) {
			closeJob.schedule((long)(delayClose*percentTime));
		}
	}
